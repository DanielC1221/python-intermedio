...
def alta(nombre, apellido, plan, precio, tree):
    ...
    if(re.match(patron, cadena1) and re.match(patron, cadena2) and re.match(patron, cadena3)):
        ...
        showerror("Error","Error en campo NOMBRE, APELLIDO o PLAN")
...
root = Tk()
root.title("Sistema para Registrar Clientes de Internet")
...
plan=Label(root, text="Plan")
plan.Combobox(row=3, column=0, sticky=W)
precio=Label(root, text="Precio")
precio.grid(row=4, column=0, sticky=W)

# Defino variables para tomar valores de campos de entrada
a_val, b_val, c_val, d_val = StringVar(), StringVar(), StringVar(), DoubleVar()
w_ancho = 20
...
boton_alta=Button(root, text="Alta", command=lambda:alta(a_val.get(), b_val.get(), c_val.get(), d_val.get(), tree))
